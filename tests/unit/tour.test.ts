...
const CONSOLE_CONTROLS: { control: string; via: string }[] = [
  ...
  { control: "a source row", via: ".tn-src-row" },
  { control: "＋ place-as-widget button", via: ".tn-src-add" },
  { control: "map on/off switch", via: ".tn-src-toggle" },
  { control: "camera feed + region filters", via: ".tn-cam-filters" },
  { control: "source section headings", via: ".tn-src-sec-head" },
