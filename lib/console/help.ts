...
const WIDGET_EXPLAINERS: WidgetExplainer[] = [
  ...
  // --- Aviation / space -----------------------------------------------------
  {
    ...
    method:
      "Read from adsb.lol, a community ADS-B receiver network, pulled worldwide by ICAO type designator: four paced requests, each naming a batch of common airliner, regional, business and light types, rather than one global snapshot or a swept grid of regions. Altitude and squawk are the aircraft's own reported values, and squawks 7500/7600/7700 raise a critical alert. TYPE is adsb.lol's own broadcast ADS-B emitter category when the aircraft transmits one we recognise — a transmitted fact, not a guess. Only when no usable category is present does it fall back to our own guess from altitude, speed and on-ground state; either way the dossier renders it with a \"· est.\" badge.",
    confidence: "measured",
    coverage:
      "Worldwide wherever volunteer receivers hear the aircraft, and only for the listed types: an aircraft broadcasting a type outside the list, or no type code at all, is never asked for, so every count is a lower bound. Dense over North America and Europe, thin to empty over oceans, deserts and much of Africa, Asia, South America and Oceania. The map draws a proportional spatial sample of the pull, so every region keeps its share rather than the busiest regions crowding the rest out.",
    limitations: [
      ...
      "There is NO military flag on this panel. The alert rule supports one, but this feed carries no military classification and does not ask for military types, so that alert can never fire here — military traffic appears only on the separate Military aircraft signal layer.",
      "adsb.lol rate-limits requests per IP, so one snapshot is cached and shared across the whole deployment and refreshed roughly every four minutes. Rows are minutes old by design, and on a bad day the panel serves the last good snapshot rather than nothing.",
