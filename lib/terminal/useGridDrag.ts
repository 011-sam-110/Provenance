...
export function useGridDrag(containerRef: React.RefObject<HTMLElement | null>) {
  ...
  const onPointerUp = useCallback((e: PointerEvent) => {
    ...
    // Release the suspension FIRST, so the board is persisted once from the state
    // the gesture actually finished in. This runs for `pointercancel` too — that
    // handler is this same function — so an interrupted drag is written rather
  ...
  const start = useCallback(
    (e: React.PointerEvent, id: string, rect: GridRect, mode: DragMode, dir: ResizeDir | null) => {
      ...
      // Suspend the STORAGE writes for the length of the gesture — never the
      // subscriber notification, which every commit below still needs in order to
      // repaint under the pointer. See THE GESTURE SUSPENSION in lib/console/store.
      // Paired with endGesture() in onPointerUp, which also handles pointercancel.
