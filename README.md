...
- **Clickable countries with a sourced dossier** — bundled Natural Earth borders outline and label every country on the satellite and topographic basemaps (which carry no labels of their own). Clicking one opens identity facts plus the Country Instability Index (flagged as a derived estimate, with each contributing layer linked) and the country-coded signals currently active there. A third section, an aggregate 0–5 government travel advisory, is built and wired but currently **dormant**: its keyless upstream (travel-advisory.info) stopped resolving, so production returns `{"advisory":null}` and the section is omitted rather than faked. Every number links to the upstream it came from.
