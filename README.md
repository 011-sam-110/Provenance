...
_Status: live at [provenance-online.vercel.app](https://provenance-online.vercel.app) and runs locally with no keys. Coverage is real but partial and depends on public upstreams staying open, so here is what production actually returned on **2026-08-13**:_

| Check | Result |
|---|---|
| all 37 `GET /api/signals/<id>` | **37 of 37 answered `200`** - 33 returned data, 4 were empty |
| `GET /api/coverage` | **6,602 cameras, 6,358 online** - from 10 of the 11 networks |
| `GET /api/planes` | **3,000 aircraft**, declared as a cap on the 3,652 that were seen |
| `GET /api/satellites` | 157 satellites |

The four empty layers were ACLED (the free account's API read access is not activated yet), ReliefWeb and ENTSO-E grid load (each waiting on a free key), and AIS ships (zero vessels available at that moment). All four still answered `200` with an empty set, which is the contract: a dormant upstream degrades to an honest blank, never a 5xx and never invented data.

**Open issue, stated plainly.** The camera count reads 6,602 rather than the roughly 17,000 those eleven networks add up to, because the Castle Rock 511 adapter, which fans out to nine US and Canadian 511 systems, is currently returning nothing from the deployment. The same adapter fetched **10,859 cameras in 18.5 s from a home IP** on the same day, and `fl511.com`, `511ny.org` and `511on.ca` each answered `200` in under two seconds, so this looks like the deployment's egress being refused rather than a dead upstream. Not yet diagnosed. Two smaller gaps: `ships` and `weather` are disabled "soon" rows in the core layer rail (both ship as *signal* layers, so those rail entries are a separate, unbuilt basemap treatment), and best-accuracy photo geolocation wants a local GeoCLIP sidecar, without which `/locate` falls back to a vision-model estimate.

Every figure above will drift. `CLAUDE.md` holds the command to re-measure each one, and [`docs/API_KEYS.md`](docs/API_KEYS.md) holds the canonical env-var names.
...
npm test                    # 1,726 unit tests across 229 files (Vitest), collected 2026-08-13
