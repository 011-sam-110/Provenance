...
| Unit tests | **4,206 cases / 398 files (2026-09-18)** | `npx vitest list` (collects without running — safe alongside other agents). Measured on feat/news-compare-board off origin/main at 8727669, which is the tree that ships. The news comparison board added two files (news-diversity, news-framing). The figure moved three times on 2026-09-16 alone — 4,029 / 389, then 4,044 / 390, then this — as the news board, the inspector rail, the area-colour picker and the news-depth branch each landed their own test files. This row said **1,414 / 215** until 2026-08-11: the suite had **more than doubled** while the table went on stating the old figure. Exactly the silent rot the header of this section warns about, and a reminder that "unpinned" here means "will be wrong", not "might be". |
